...
const PrivateMessages = () => {
  ...
  if(isLoading)
	...
	if (usersMessages.length === 0)
		return (
			<div>No messages yet. Send them from the mail icon on users trends.</div>
		);
					...
					<>
						<div className='message-info-container'>
							...
							<div className='from-user-and-date-created'>
								<span className='from-user'>@{message.from_user} on "{message.trend_title}"</span>
								<span className='date-created'>
									Date:{' '}
									{moment(message.created_at).format('MMMM Do YYYY, h:mm a')}
								</span>
							</div>
						</div>
					</>
